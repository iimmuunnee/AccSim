...
시작은 유튜브 영상이었습니다. AI 가속기라는 개념을 처음 접했고, AI가 일상이 될수록 "그 AI가 어떤 칩 위에서 도는가"는 개발자만의 지식이 아니게 될 거라고 생각했습니다. 그래서 이 지식을 누구나 관람할 수 있는 형태로 전달하고 싶었습니다 — 평소 전시회를 좋아하기도 했고, **웹으로 만든 전시관**이라는 형식이 참신하면서도 일반인에게 문턱이 낮겠다고 판단했습니다. 시뮬레이터를 만들고 그 내부를 9개의 전시관으로 연 이유입니다. 전시의 소재가 필요했을 때 직전 프로젝트 [SolarX](https://github.com/iimmuunnee/SolarX)의 LSTM 모델을 가상 칩에 올리면서, 두 프로젝트가 자연스럽게 이어졌습니다.

### HW-SW Co-design 역량 시연
