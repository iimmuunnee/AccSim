...
**라이브 데모를 직접 체험해보세요: [https://acc-sim.vercel.app](https://acc-sim.vercel.app)**
...
> 배포된 사이트에서 바로 체험할 수도 있습니다: **[https://acc-sim.vercel.app](https://acc-sim.vercel.app)**
...
| 서비스 | 플랫폼 | URL |
|--------|--------|-----|
| **Frontend** | Vercel | [https://acc-sim.vercel.app](https://acc-sim.vercel.app) |
| **Backend** | Render | [https://accsim.onrender.com](https://accsim.onrender.com) |

**환경변수:**

| 변수 | 위치 | 설명 |
|------|------|------|
| `NEXT_PUBLIC_API_URL` | Frontend (Vercel) | 백엔드 API URL (`https://accsim.onrender.com`) |
| `CORS_ORIGINS` | Backend (Render) | 허용할 프론트엔드 origin (`https://acc-sim.vercel.app`) |
...
### 6. 3D Canvas 휠 스크롤과 스냅 스크롤 충돌

**Problem**: HALL 3(ChipHall)의 3D Systolic Array 시각화에서 OrbitControls의 줌(휠 스크롤)과 페이지의 스냅 스크롤이 충돌했습니다. Canvas 위에서 휠을 굴리면 3D 줌과 섹션 전환이 동시에 발생하는 버그였습니다.

**Cause**: `useSnapScroll.ts`의 `handleWheel`이 컨테이너의 모든 휠 이벤트를 캡처하여 섹션 전환을 트리거했습니다. Canvas 위에서 발생한 휠 이벤트도 bubbling되어 SnapContainer까지 도달하므로, OrbitControls의 줌과 스냅 스크롤 전환이 동시에 실행되었습니다.

**Solution**: `handleWheel` 함수에서 `e.target`을 검사하여, `canvas` 요소(또는 그 내부)에서 발생한 휠 이벤트면 early return하도록 수정했습니다. `e.preventDefault()`도 호출하지 않으므로 OrbitControls가 정상적으로 줌을 처리하고, Canvas 밖 영역의 스냅 스크롤은 기존과 동일하게 동작합니다.

```ts
// useSnapScroll.ts — handleWheel 내부
const target = e.target as HTMLElement
if (target.closest('canvas')) return  // Canvas 위 휠 → 스냅 스크롤 무시
```
